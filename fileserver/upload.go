...
func (fServer *FileServer) writeRecord(f *File) error {
	uuid := f.UUID
	_, err := os.Stat(FileStoreDir + uuid)
	if errors.Is(err, os.ErrExist) {
		logger.Fatal(
			"This should not happen. File exists in store but not in cache!",
			"fileserver.writeRecord",
			err,
		)
		return err
	}
	...
	if !json.Valid(rData) {
		logger.Fatal(
			"Json generated by the new record is NOT valid",
			"writeRecord",
...
func cleanTmp(url string) error {
	if err := os.Remove(TmpDir + "DAT_" + url); err != nil {
